#include <cstring>
#include "Yux-dagger-F_p.h"

uint64_t Yux_dagger_F_p::Model_p(uint64_t state)
{
  if(state < 0)
  {
...
void Yux_dagger_F_p::addRoundKey(uint64_t state[], uint64_t RoundKey[], int round)
{
  int i;
...
void Yux_dagger_F_p::subtractRoundKey(uint64_t state[], uint64_t RoundKey[], int round)
{
  int i;
  for(i=0;i<BlockWords;i++)
    {
...
void Yux_dagger_F_p::decSboxFi(uint64_t state[], int begin)
{
...
void Yux_dagger_F_p::decLinearLayer(uint64_t in[16])
{
    int j;  
    uint64_t temp[16];
    for(j=0;j<16;j++){
    ...
    for(j=0; j<16; j++){
      in[j] =    (modulus-temp[j]
                ...
                + temp[(j+6)%16]
                + temp[(j+10)%16]
                + temp[(j+12)%16]) % modulus;
      }
}

// Cipher is the main function that encrypts the PlainText.
void Yux_dagger_F_p::decryption(uint64_t out[], uint64_t in[], uint64_t RoundKey[])
{
  int i, r;
...
void Yux_dagger_F_p::encSboxFi(uint64_t state[], int begin)
{
  uint64_t c0=state[begin];
  uint64_t c1=state[begin+1];
  uint64_t c2=state[begin+2];
...
void Yux_dagger_F_p::encLinearLayer(uint64_t in[16])
{
    uint64_t temp[16];
    int j;
    ...
    in[j] =   ( 
                ((temp[j]) * (modulus-4)) +
                ((temp[(j+2)%16]) * (modulus-2))+
                ((temp[(j+4)%16]) * 4) % modulus+
                ((temp[(j+6)%16] + temp[(j+14)%16]) * 2) +
                ((temp[(j+8)%16])) +
                ((temp[(j+10)%16]) * 3) +
                ((temp[(j+12)%16]) * (modulus-1)) 
              )  * 30584 % modulus;                               
    }
...
// Cipher is the main function that encrypts the PlainText.
void Yux_dagger_F_p::encryption(uint64_t out[], uint64_t in[], uint64_t RoundKey[])
{
  int i, r;
  // initial key addition
  // Add Round key before first round
...
void Yux_dagger_F_p::constantForKey(uint64_t RC[56][4])
{
...
// array a, length = l, <<<3
void Yux_dagger_F_p::rotation(uint64_t *a, int l,int r)
{
	uint64_t temp[l];
	for (int i = 0; i < l; i++){
		temp[i] = a[(i+r)%l];
...
// This function produces Nb(Nr+1) round keys.
// The round keys are used in each round to encrypt the states.
long Yux_dagger_F_p::KeyExpansion(uint64_t RoundKey[], uint64_t Key[])
{
    // Nr is the round number
    // Nk is the number of 64-bit Feistel works in the key 4bytes each round.
    int Nr = ROUND;
...
void Yux_dagger_F_p::decRoundKey(uint64_t RoundKey_invert[], uint64_t RoundKey[])
{
    int Nr = ROUND;
    int Nk = BlockWords; // 128/8= 16
