#include "transciphering-dagger-F_p-16.h"

using namespace helib;
using namespace std;
using namespace NTL;

// static long pROUND1 = 9;

Transcipher16_dagger_F_p::Transcipher16_dagger_F_p(std::shared_ptr<helib::Context> con)
    : context(con),
      he_sk(*context),
      ea(context->getEA()) {
  
  cout << "Transcipher16_dagger_F_p init ..." <<endl;
...
shared_ptr<Context> Transcipher16_dagger_F_p::create_context(
...
int Transcipher16_dagger_F_p::print_noise(vector<Ctxt>& ciphs) {
...
void Transcipher16_dagger_F_p::print_parameters() {
  if (!context) {
...
void Transcipher16_dagger_F_p::create_pk() {
  he_pk = std::make_unique<helib::PubKey>(he_sk);
  return;
}

helib::EncryptedArray Transcipher16_dagger_F_p::getEa() {
  return ea;
}

//----------------------------------------------------------------
// encrypt the expanded key roundKeySchedule.
void Transcipher16_dagger_F_p::encryptSymKey(vector<Ctxt>& eKey, vector<uint64_t>& roundKeySchedule)
{   
...
void Transcipher16_dagger_F_p::FHE_YuxDecrypt(vector<Ctxt>& eData, const vector<Ctxt>& symKey) 
{
...
void Transcipher16_dagger_F_p::FHE_YuxDecrypt(vector<Ctxt>& eData, const vector<Ctxt>& symKey, const Vec<uint64_t> inBytes)
{
  {
...
// packing
void Transcipher16_dagger_F_p::encodeTo16Ctxt(Vec<ZZX>& encData, const Vec<uint64_t>& data, long s)
{
  // 1 block has 16 word
...
void Transcipher16_dagger_F_p::buildRoundConstant(Ctxt& encA)
{
  // long --> ZZX -->Ctxt 
...
void Transcipher16_dagger_F_p::decSboxFunc(vector<Ctxt>& eData, long begin, Ctxt& encA)
{
    Ctxt c0(eData[begin]);
...
void Transcipher16_dagger_F_p::decrypt(helib::Ctxt& in, uint64_t& out) {
  std::vector<long> p;
